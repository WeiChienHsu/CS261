 ...
 * deleteDynArr:  Deallocate data array and the dynamic array ure. 
 * _dynArrSetCapacity: the underlying array to be the size cap.
 ...
 * putDynArr: Put an item into the dynamic array at the specified location, overwriting 
...
{
	/* Chech if DynArr need an additional space for this new element */
	if(sizeDynArr(v) == v -> capacity) {
		/* Double our DynArr */
		_dynArrSetCapacity(v, (v -> capacity) * 2);
	}

	v -> data[v -> size] = val; /* assign the value by indexing the position of current size */
	v -> size ++;	/* Update the size by 1 */
...
{
	assert(pos < v -> size && pos >= 0); /* Chech if pos is valid */
	return v -> data[pos];
}
...
{
	/* Chech if those two indexs are valid */
	assert(i < v -> size && i >= 0);
	assert(j < v -> size && j >= 0);

	/* use a temporay TYPE element to store value of first element */
	TYPE temp = v -> data[i];
	v -> data[i] = v -> data[j];
	v -> data[j] = temp;
}
...
{
	assert(idx < v -> size && idx >= 0); /* Check if idx is valid */
	v -> size --; /* Update the size */

	if(idx == (v -> size - 1)) {
		/* If the removed index equal the last index which means we don't need to move old elements */
		v -> data[idx] = 0;
	} else {
		/* From the index of idx, all elements before it will be moved to privious index */
