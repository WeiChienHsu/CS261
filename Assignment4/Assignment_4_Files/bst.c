...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if(cur == 0) {
        ...
        /* Assign value to the new Node and points its children to 0 */
        newNode -> val = val;
        newNode -> left = 0;
        newNode -> right = 0;
    ...
    if(compare(val, cur -> val) == - 1) {
        /* Target value is smaller than the current value, move left */
        cur -> left = _addNode(cur -> left, val);
    }
    else if(compare(val, cur -> val) == 1) {
        /* Target value is larget than the current value, move right */
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while(currentNode != NULL) {
        ...
        if(compare(val, currentNode -> val) == 0) {
            /* Find the target, return 1 */
            return 1;
        }
        else if(compare(val, currentNode -> val) == -1) {
            /* Target is smaller than current node value */
            currentNode = currentNode -> left;
        }
        else {
            /* Target is larget than current node value */
            currentNode = currentNode -> right;
        }
    }
    /* After traversal, still not found target */
    return 0;
}
...
{
    /* Traverse to the left children until leftMost is found */
    while(cur -> left != 0) {
        cur = cur -> left;
    }
...
struct Node *_removeLeftMost(struct Node *cur)
{
    if(cur != 0) {
        ...
        remove the node itself and return right child */
        if(cur -> left == 0) {
            free(cur); /* Remove the left most */
            return cur -> right; /* Search to the right child */
        }
        else {
            /* Keep searching left child until meet the end */
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(cur -> val, val) == 0) {
        /* Find the LeftMost value in the right child 
        to replace value of removed node */
        if(cur -> right == 0) {
            /* if there is no right child, remove the current node */
            /* Connect current node's left child with original tree */
            struct Node* temp = cur -> left;
            free(cur);
            return temp;
        }

        /* If there is a right child, find the left Most value of it */
        cur -> val = _leftMost(cur -> right);
        /* Remove the left most node in the right child*/
        cur -> right = _removeLeftMost(cur -> right);

    }
    else if (compare(cur -> val, val) == -1) {
        /* Target is larger then current node value, go right */
        cur -> right = _removeNode(cur -> right, val);
    }
    else {
        /* Target is smaller then current node value, go left */
